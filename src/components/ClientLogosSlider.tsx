...
export function ClientLogosSlider({ title = "Trusted by leading brands", dark = false }: { title?: string; dark?: boolean }) {
  ...
  return (
    <section className={`py-16 overflow-hidden border-b ${dark ? "bg-[#39bcfc] border-white/10" : "bg-[#d6f2ff] border-[#87d4f8]"}`}>
      ...
      <div className="relative w-full flex items-center">
        {/* Edge fades */}
        <div className={`absolute left-0 w-32 h-full bg-gradient-to-r ${dark ? "from-[#39bcfc]" : "from-[#d6f2ff]"} to-transparent z-10`} />
        <div className={`absolute right-0 w-32 h-full bg-gradient-to-l ${dark ? "from-[#39bcfc]" : "from-[#d6f2ff]"} to-transparent z-10`} />
